...
| 2025-10-19 21:24  | Pinned tfsec action version                 | Updated validation workflow (then `terraform-validate`, now `prepare-for-demo`) to use `aquasecurity/tfsec-action@v1.0.0`. | ~1 min     |
...
| 2025-10-20 19:05  | Added manual gates to demo workflow          | Updated `.github/workflows/terraform-validate.yml` (now `prepare-for-demo.yml`) to require approvals before demo apply/destroy stages. | ~3 min     |
...
| 2025-10-20 19:52  | Switched demo approvals to environments      | Replaced issue-based approvals with environment-gated apply/destroy jobs in the validation workflow (now `prepare-for-demo`). | ~4 min     |
